...
def plotCamera(cameraPos, im, centers=[3000,0,-500], span=np.ones(3, dtype=np.float)*6000, color=(255,255,255)):
	rez = im.shape
	...
	cv2.circle(im, (y,x), 5, color, thickness=-1)

def viewImage(cam_index, filetime, coms=None):
	''' Configs '''
	colors = [(255,0,0),(0,255,0),(0,0,255)]
	URIs_depth = ['/media/Data/ICU_Dec2012/ICU_Dec2012_r40_c1/depth/'+filetime[0]+'/'+filetime[1]+'/'+filetime[2]+'/device_1/depth_'+'_'.join(filetime)+'.png',
	...
	URIs_skel = ['/media/Data/ICU_Dec2012/ICU_Dec2012_r40_c1/skel/'+filetime[0]+'/'+filetime[1]+'/'+filetime[2]+'/device_1/skel_'+'_'.join(filetime)+'_.dat',
			'/media/Data/ICU_Dec2012/ICU_Dec2012_r40_c2/skel/'+filetime[0]+'/'+filetime[1]+'/'+filetime[2]+'/device_1/skel_'+'_'.join(filetime)+'_.dat',
			'/media/Data/ICU_Dec2012/ICU_Dec2012_r40_c2/skel/'+filetime[0]+'/'+filetime[1]+'/'+filetime[2]+'/device_2/skel_'+'_'.join(filetime)+'_.dat']
	
	color = colors[cam_index]
	im = sm.imread(URIs_depth[cam_index])
	fillImage(im)
	
	try:
	# if 1:
		# embed()
		with open(URIs_skel[cam_index], 'rb') as inFile:
			import pickle
			users = pickle.load(inFile)	
			for u in users.keys():
				skel = users[u]['jointPositions'].values()
				if skel[0] is not -1:
					skel = world2depth(np.array(skel), [240,320])
					im = display_MSR_skeletons(im, skel, color=color, skel_type='Kinect')

	except:
		print "Error showing skeleton"
		embed()
		pass

	if coms is not None:
		c = coms
		cv2.circle(im, (c[0],c[1]), 5, 5000, thickness=-1)
...
'''
Transform12 = np.array([[ -8.53119523e-01,  -8.21532038e-02,   5.15206688e-01,	0.96222998e+03],#1.66222998e+03
						[  1.57073609e-01,   9.01259156e-01,   4.03806655e-01, 	1.79686253e+02],#.79686253e+02
						[ -4.97508755e-01,   4.25420714e-01,  -7.55977681e-01,	-3.68191742e+03],#-3.68191742e+03
						[  0.00000000e+00,   0.00000000e+00,   0.00000000e+00,	1.00000000e+00]])

Transform32 = np.array([[  9.55378205e-01,  -9.69196766e-02,   2.79032365e-01, -0*5.93818783e+02], #5.93818783e+02
					    [ -8.66849294e-03,   9.35032463e-01,   3.54456133e-01, 1.78054818e+02], #1.78054818e+02
	 					[ -2.95258094e-01,  -3.41058454e-01,   8.92469489e-01, 0*.1910900e+02], #-1.89910900e+02
	  				 	[  0.00000000e+00,   0.00000000e+00,   0.00000000e+00,1.00000000e+00]])
...
userCount = 0
t = timeStart
# indsPrev = [3000]*cameraCount ############ remove
print 'm', len(times1),len(times2),len(times3)
...
while 1:
	...
	for u in currentUsers:
		for cam_index in range(cameraCount):
			...
			if dists[closestPerson] < 500:
				...
				centroid = world2depth(np.array([u['data'][-1]['com']]), [240,320])[0] #rez=[320,240]
				print centroid

				try:
					print u['data'][-1]['time']
					# embed()
					im_tmp = viewImage(u['cameras'][-1], u['data'][-1]['time'], coms=centroid)
					ims.append(im_tmp)
				except:
	...
	plotCamera(cameraCenter1, mapIm, centers=centers, color=(255,0,0))
	plotCamera(cameraCenter2, mapIm, centers=centers, color=(0,255,0))
	plotCamera(cameraCenter3, mapIm, centers=centers, color=(0,0,255))
