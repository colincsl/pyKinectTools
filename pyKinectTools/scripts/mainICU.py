...
# if 1:
	...
	if 1:
		...
		# if 1:
			tStart = time.time()
			reader1.run()	
			depthRaw = reader1.depthDataRaw
			depthRaw8 = constrain(depthRaw, 500, 2000)
			imgs.append(deepcopy(depthRaw8))
			...
			if len(labelInds1) > 0:
				...
				ids = tracker1.run(com1_xyz, objects1, t, reader1.depthFilename, touched, vecs1, ornCompare)
				# highlight = generateKeypoints(objects1, labelInds1, out1, depthRaw, com1, featureExt1)
			...
			# 	try:
			# 		extrema = getExtrema(objects1, labelInds1, out1, depthRaw, com1, featureExt1, i)
			...
			if vizWin:
				...
				depthRaw8 = constrain(depthRaw, 500, 4000)
				depthRaw8 = np.dstack([depthRaw8, depthRaw8, depthRaw8])
				# Draw binary sensors
				if 0:
					for i in featureExt1.touchAreas:
						...
						r = int(i[1]*0.27)
						cv2.circle(depthRaw8, (center[1][0],center[0][0]), r, [0,100, 100], thickness=2)

				for i in xrange(len(labelInds1)):
					# out1[out1==labelInds1[i]] = (ids[i]+1)*50
					if 1:
						depthRaw8[out1==labelInds1[i], ids[i]%3] = (ids[i]+1)*50
						depthRaw8[out1==labelInds1[i], (ids[i]+1)%3] = 0
						depthRaw8[out1==labelInds1[i], (ids[i]+2)%3] = 0
					if 0:
						# print "t", touched
						depthRaw8[out1==labelInds1[i], 1] = 75 * (ids[i]%3 == 0)
						depthRaw8[out1==labelInds1[i], 2] = 75 * (ids[i]%3 == 1)
						depthRaw8[out1==labelInds1[i], 0] = 75 * (ids[i]%3 == 2)

						for j in range(len(touched)):
							t = [labelInds1[x] for x in touched[j]]

							if labelInds1[i] in t:
								depthRaw8[out1==labelInds1[i], 1] = 255 * (ids[i]%3 == 0)
								depthRaw8[out1==labelInds1[i], 2] = 255 * (ids[i]%3 == 1)
								depthRaw8[out1==labelInds1[i], 0] = 255 * (ids[i]%3 == 2)
								# Draw bigger radius
								center = featureExt1.touchAreas[j][0]
								center = world2depth(np.array([center]))
								r = int(featureExt1.touchAreas[j][1]*.1)
								cv2.circle(depthRaw8, (center[1][0],center[0][0]), r, [0,150, 150], thickness=4)

				if len(labelInds1) > 0:
					depthRaw8[highlight[0]-5:highlight[0]+6, highlight[1]-5:highlight[1]+6,:] = 255

				# for exSet in extremaInds:
				# 	for ex in exSet:
				# 		depthRaw8[ex[0]-2:ex[0]+2, ex[1]-2:ex[1]+2] = 255

				cv2.imshow("a", depthRaw8)
				out1 *= 10 * (out1>0)
...
if 0:
	...
	while (len(reader1.allPaths) > 0):
		try:
			...
			depthRaw = reader1.depthDataRaw
			depthRaw8 = constrain(depthRaw, 500, 2000)
			depthRaw8[depthRaw8==depthRaw8.max()] = 0
			diff = np.array(m1, dtype=int16) - np.array(depthRaw8, dtype=int16)
			diff *= (depthRaw!=0)*(depthRaw<2000)
			...
			# scipy.misc.imsave(fold+"depthRaw/"+str(i)+".jpg", depthRaw)
			# scipy.misc.imsave(fold+"depthRaw8/"+str(i)+".jpg", depthRaw8)
			scipy.misc.imsave(fold+"diffDraw/"+str(i)+".jpg", diffDraw)
	...
	## Adapt background
	# if len(objects1) == 0:
	# 	depthImgs1[:,:,depthStackIndepthRaw] = depthRaw
	# 	mean1 = getMeanImage(depthImgs1)
	# 	m1 = constrain(mean1, 500, 4000)
	# 	depthStackIndepthRaw += 1
	# 	if depthStackIndepthRaw == 5:
	# 		depthStackIndepthRaw = 0
	# if len(objects2) == 0:
...
# np.savez('tmpPerson_close.npz', {'objects':objects1, 'labels':labelInds1, 'out':out1, 'd':depthRaw, 'com':com1, 'features':featureExt1})
...
if 0:
	...
	## See numpy-sharedmem
		...
		# # Camera 1
		# depthRaw = reader1.depthDataRaw
		# diff1 = depthRaw*(np.abs(depthRaw - mean1) > 200)
		# diffDraw = constrain(diff1, 500, 4000)
		# d2 = reader2.depthDataRaw
