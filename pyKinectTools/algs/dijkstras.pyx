...
cdef inline int dim2ind(int i, int j, int rezX, int rezY): return (i*rezX+j)
...
cpdef inline list graphDijkstras(np.ndarray[INT16, ndim=2] dists2Tot, np.ndarray[UINT8, ndim=2] visitMat, np.ndarray[INT16, ndim=3] dists2, list current_):
	...
	cdef unsigned int currentCost, direc=0,prevDirec, found, checkAll
	cdef int tmp, tmpMin=0, tmpNeigh=0
	cdef int currentVal
	cdef int VISITED = 254, OUTOFBOUNDS=255, TOUCHED=1
	cdef int vTMP = 250
	...
	# for i in range(length):
		...
		''' Check if in bounds. If not find new node that hasn't been visited. '''
		if (current[0] < 1) or (current[0] > shape_[0]-1) or (current[1] < 1) or (current[1] > shape_[1]-1):
			found = 0
			for x in range(shape_[0]):
				for y in range(shape_[1]):	
					if visitMat_c[x][y] == TOUCHED: # or dists2Tot_c[x][y] == 0:
						current[0] = x
						current[1] = y
						found = 1
						break
		...
		for j in range(4):
			...
			visitMat_c[neigh[0][j]][neigh[1][j]] = int_max(TOUCHED, visitMat_c[neigh[0][j]][neigh[1][j]])
			# dists2Tot_c[neigh[0][j]][neigh[1][j]] = int_min(dists2Tot_c[neigh[0][j]][neigh[1][j]], (dists2_c[current[0]][current[1]][j] + currentCost +1))
			dists2Tot_c[neigh[0][j]][neigh[1][j]] = int_min(dists2Tot_c[neigh[0][j]][neigh[1][j]], (dists2_c[current[0]][current[1]][j] + currentCost))


			# if dists2Tot_c[neigh[0][j]][neigh[1][j]] == 32000:
				# print dists2Tot_c[neigh[0][j]][neigh[1][j]], (dists2_c[current[0]][current[1]][j] + currentCost +1)
				# dists2Tot_c[neigh[0][j]][neigh[1][j]] = 0

			# print int_min(dists2Tot_c[neigh[0][j]][neigh[1][j]], (dists2_c[current[0]][current[1]][j] + currentCost +1))
			# if visitMat_c[neigh[0][j]][neigh[1][j]] == OUTOFBOUNDS:
