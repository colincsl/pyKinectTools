...
def compute_normals_with_radius(posMat, radius=3):
    ...
    normals : ndarray (NxMx3)
        Image with normal vectors for each pixel
    """
    print "compute_normals_with_radius() is deprecated: \
            This function is much slower than compute_normals()"
...
def getTopdownMap(depthMap, rotation=None, rez=1000, centroid=[0, 0, 0], bounds=[]):
    ...
    indsNew = np.asarray([np.round((xyzNew[:, 0] - xyzMin[0]) / (xyzMax[0] - xyzMin[0])*(rez-1)),
                         np.round((xyzNew[:, 1] - xyzMin[1])/(xyzMax[1]-xyzMin[1])*(rez-1))],
                         dtype=np.int)
...
def tabletop_detector(im_depth, bg_threshold=1000, subsample=1, im_pos=None,
                      im_norm=None, thresh=30, n_iter=100):
    """
    --Input--
    im_depth : ndimage
    bg_threshold : int
    ...
    plane_offset, plane_normal, _ = plane_detection(im_pos[::subsample, ::subsample],
                                                    im_norm[::subsample, ::subsample],
