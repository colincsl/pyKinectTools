...
class MSRPlayer(BasePlayer):
	...
	def __init__(self, base_dir='./', get_depth=True, get_color=True,
		...
		self.labels = [i for i in it.product(actions, subjects, positions)]
	...
	def run(self):
		...
		while len(self.filenames) > 0:
			if len(self.filenames) > 0 and self.depth_stack is None:
					...
					self.action_label, self.user_label, self.position_label = self.labels.pop(0)

			while self.current_frame < framecount:
